-- Migration to ensure title column exists and is properly configured
-- This script adds the title column if it doesn't exist and sets up proper defaults
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Update existing records to have default titles based on target temperature and date
  ...
  -- Ensure temperature_entries column exists with proper structure
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'firing_logs'
  ...
  -- Ensure warning_flags column exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'firing_logs'
...
COMMENT ON TABLE public.firing_logs IS 'Stores detailed firing logs with temperature tracking and warnings';
